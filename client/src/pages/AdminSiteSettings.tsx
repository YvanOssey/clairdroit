...
const pageSections: PageSection[] = [
  ...
  { title: "Rubriques", group: "rubrics", fields: [
    { key: "eyebrow", label: "Surtitre" }, { key: "titleMain", label: "Titre — première ligne" }, { key: "titleAccent", label: "Titre — accent" }, { key: "intro", label: "Introduction", multiline: true }, { key: "selectionEyebrow", label: "Surtitre sélection" }, { key: "selectionTitle", label: "Titre sélection" }, { key: "emptyEyebrow", label: "Surtitre sans publication" }, { key: "emptyTitle", label: "Titre sans publication", multiline: true },
  ] },
...
function PageContentPreview({ settings }: { settings: SiteSettingsValues }) {
  const { about, featured, decryptions, rubrics, contact } = settings.pageContent;
  const previews = [
    ["À propos", about.titleMain, about.intro],
    ["À la une", `${featured.titleMain} ${featured.titleEnd}`, featured.description],
    ["Décryptages", `${decryptions.titleMain} ${decryptions.titleAccent}`, decryptions.description],
    ["Rubriques", `${rubrics.titleMain} ${rubrics.titleAccent}`, rubrics.intro],
    ["Nous écrire", `${contact.titleMain} ${contact.titleAccent}`, contact.description],
  ];
  return <div className="mt-4 grid gap-2">{previews.map(([label, title, description]) => <div key={label} className="border-l-2 border-[#b86e4b] bg-[#ece6da] px-3 py-2"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#b86e4b]">{label}</p><p className="mt-1 font-display text-base font-semibold text-[#12243b]">{title}</p><p className="mt-1 line-clamp-2 text-[10px] leading-4 text-[#667384]">{description}</p></div>)}</div>;
}
