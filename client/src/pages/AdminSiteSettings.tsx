...
export default function AdminSiteSettings() {
  ...
  const uploadMutation = trpc.articles.uploadImage.useMutation({
    onSuccess: ({ url }) => { const target = uploadTargetRef.current; setForm((current) => target === "logo" ? { ...current, logoUrl: url } : { ...current, pageContent: { ...current.pageContent, about: { ...current.pageContent.about, photoUrl: url } } }); setUploading(false); toast.success(target === "logo" ? "Logo téléversé." : "Portrait téléversé."); },
    onError: (error) => { const target = uploadTargetRef.current; setUploading(false); toast.error(error.message || `Impossible de téléverser le ${target === "logo" ? "logo" : "portrait"}.`); },
  });
  ...
  const handleUpload = (event: React.ChangeEvent<HTMLInputElement>, target: "logo" | "photo") => {
    ...
    if (file.size > 6_000_000) { toast.error("L’image ne doit pas dépasser 6 Mo."); return; }
